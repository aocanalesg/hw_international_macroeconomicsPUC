...
%read excel 
[data_nic] = readmatrix('DataNicaragua.xlsx', 'Sheet', 'Data','Range','E2:BN9');
[data_usa] = readmatrix('data_usa.xlsx', 'Sheet', 'Data','Range','E2:BI9');
...
results_std(7,:) = [std(det1_tb_nic), std(det1_tb_usa), std(det2_tb_nic), std(det2_tb_usa), std(hp_cycle_nic_5), std(hp_cycle_usa_5), std(hp_cycle_nic_11), std(hp_cycle_usa_11)]%primera fila solo incluye la desviacion por si sola
...
%%%%%% Nicaragua
fig(1) = figure('name', 'Nic','units','inch','position',[0,0,11,7.5]);
...
fig(2)= figure('name', 'USA','units','inch','position',[0,0,11,7.5]);
subplot(2,2,1);
x = 1965:2021;
...
exportgraphics(figure(2),'picture2.png','Resolution',700)
...
fig(3)= figure('name', 'NIC_REAL_GDP','units','inch','position',[0,0,11,7.5]);
plot([1960:2021], GDP_nic_constant, 'LineWidth',1.5);
xregion(1977,1983);
xregion(2008,2017);
xlabel('Año');
ylabel('Real GDP');
xlim([1960 2021]);
title('PIB real de Nicaragua 1960-2021');
...
exportgraphics(figure(4),'picture4.png','Resolution',700)
